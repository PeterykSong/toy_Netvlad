...
def format_array_preview(values, max_items=8):
...
def print_run_configuration():
    ...
    major_params = {
        ...
        'cacheBatchSize': opt.cacheBatchSize,
        'cacheRefreshRate': opt.cacheRefreshRate,
        'nEpochs': opt.nEpochs,
        'optim': opt.optim,
        'lr': opt.lr,
        'lrStep': opt.lrStep,
        ...
        'threads': opt.threads,
        'seed': opt.seed,
        'resume': opt.resume if opt.resume else 'None',
        'device': str(device),
    }
    for key, value in major_params.items():
        print('  {}: {}'.format(key, value))

def print_model_structure(current_model):
    ...
    print(current_model)
    print('===> Model Parameters: total={}, trainable={}'.format(total_params, trainable_params))

def get_netvlad_alpha(current_model):
    if opt.pooling.lower() != 'netvlad' or not hasattr(current_model, 'pool'):
        return None

    pool_module = current_model.pool
    if isinstance(pool_module, nn.DataParallel):
        pool_module = pool_module.module
...
def print_first_triplet_debug(epoch, iteration, flattened_indices, neg_counts, vlad_q, vlad_p, vlad_n):
    ...
    query_index, positive_index, first_negative_index = get_query_db_indices(flattened_indices, neg_counts)
    query_utm = train_set.dbStruct.utmQ[query_index]

    query_descriptor = vlad_q[0].detach().cpu().numpy()
    positive_descriptor = vlad_p[0].detach().cpu().numpy()
    negative_descriptor = vlad_n[0].detach().cpu().numpy()
    positive_feature_distance = np.linalg.norm(query_descriptor - positive_descriptor)
    negative_feature_distance = np.linalg.norm(query_descriptor - negative_descriptor)
    first_neg_count = int(neg_counts[0].item())
    netvlad_alpha = get_netvlad_alpha(model)

    print('==> Epoch[{}]({}) First triplet debug'.format(epoch, iteration), flush=True)
    if netvlad_alpha is not None:
        print('    netvlad_alpha={:.6f}'.format(float(netvlad_alpha)), flush=True)
    print('    query_idx={} utm={} descriptor_shape={} descriptor_head={}'.format(
        query_index,
        format_array_preview(query_utm, max_items=2),
        tuple(query_descriptor.shape),
        format_array_preview(query_descriptor),
    ), flush=True)
    print('    positive_idx={} descriptor_shape={} descriptor_head={}'.format(
        positive_index,
        tuple(positive_descriptor.shape),
        format_array_preview(positive_descriptor),
    ), flush=True)
    print('    positive_feature_distance_from_query={:.4f}'.format(positive_feature_distance), flush=True)
    print('    negative_idx={} descriptor_shape={} descriptor_head={} (first of {} negatives)'.format(
        first_negative_index,
        tuple(negative_descriptor.shape),
        format_array_preview(negative_descriptor),
        first_neg_count,
    ), flush=True)
    print('    negative_feature_distance_from_query={:.4f}'.format(negative_feature_distance), flush=True)
